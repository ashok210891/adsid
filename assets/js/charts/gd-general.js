...
!function (NioApp, $) {
  ...
  var salesOverview = {
    ...
    datasets: [{
      color: "#798bff",
      background: NioApp.hexRGB('#798bff', .3),
      data: ["", "", "", 5500, 9200, 9690, 8200, 7800, 9500, 5500, 9200, 9690, 8200, 7800, 9500, 5500, 9200, 9690, 8200, 7800, 9500, 5500, 9200, 9690, 8200, 7800, 9500, 5500, 9200, 9690]
    }]
  };
  ...
  function lineSalesOverview(selector, set_data) {
    ...
    var req = new Request();
    req.url = "currentmonthusers";
    RequestHandler(req, function(data){

      var d = new Date();
      var currentmonth = d.getMonth();
      var currentyear = d.getFullYear();
      const daysserial = getDaysInMonth(currentmonth,currentyear)
      var currentmonthuserscount = 0;
      var mydata = JSON.parse(data);

      for(var j=0;j<mydata.length;j++){
        currentmonthuserscount = (+currentmonthuserscount) + (+mydata[j].countdata);
      }
      $("#currentmonthuserscount").html(currentmonthuserscount);
      $("#dayscount").html(daysserial.length);

      var chartdata = [];
      for(var i=0;i<daysserial.length;i++){
        var checkdate = 0;
        var currentdatedata = 0;
        for(var j=0;j<mydata.length;j++){
          if(daysserial[i] == mydata[j].entrydate){
            checkdate = 1;
            currentdatedata = mydata[j].countdata;
            break;
          }
        }
